...
# Authors: David Kainer and Kyle Sullivan
########################################################################
...
parse_arguments <- function() {
  ...
  option_list = list(
    ...
    make_option(c("--simple-filenames"),
                action="store_true",
                default=FALSE,
...
nullDist <- function(geneset, adjnorm, mpo, restart = 0.7, tau = 1, name = NULL, threads = 1, verbose = NULL) {
  ...
  for (i in 1:numRand) {
    ...
    if (progress %% 10 == 0) message(sprintf('%s%% complete', progress))
  }
...
main <- function() {
  ...
  suppressPackageStartupMessages({
    ...
    library(foreach)
    library(signal)
    # Possibly fork repo for install??
